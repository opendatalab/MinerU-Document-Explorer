...
describe("CLI Skill Commands", () => {
  test("shows embedded skill with --skill alias", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toContain("MinerU Document Explorer Skill (embedded)");
    expect(stdout).toContain("name: mineru-document-explorer");
  ...
  test("installs the skill into the current project", async () => {
    ...
    const skillDir = join(projectDir, ".agents", "skills", "mineru-document-explorer");
    expect(readFileSync(join(skillDir, "SKILL.md"), "utf-8")).toContain("name: mineru-document-explorer");
    expect(readFileSync(join(skillDir, "references", "mcp-setup.md"), "utf-8")).toContain("Claude Code");
    expect(existsSync(join(projectDir, ".claude", "skills", "mineru-document-explorer"))).toBe(false);
    expect(stdout).toContain(`✓ Installed skill to ${skillDir}`);
  ...
  test("installs globally and creates the Claude symlink with --yes", async () => {
    ...
    const skillDir = join(fakeHome, ".agents", "skills", "mineru-document-explorer");
    const claudeLink = join(fakeHome, ".claude", "skills", "mineru-document-explorer");

    expect(readFileSync(join(skillDir, "SKILL.md"), "utf-8")).toContain("name: mineru-document-explorer");
    expect(lstatSync(claudeLink).isSymbolicLink()).toBe(true);
    expect(readFileSync(join(claudeLink, "SKILL.md"), "utf-8")).toContain("name: mineru-document-explorer");
    expect(stdout).toContain(`✓ Installed skill to ${skillDir}`);
    expect(stdout).toContain(`✓ Linked Claude skill at ${claudeLink}`);
  });

  test("skips Claude symlink when .claude/skills already points to .agents/skills", async () => {
