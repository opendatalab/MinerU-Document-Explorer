...
describe("MCP Server", () => {
  ...
  describe.skipIf(!!process.env.CI)("hybridQuery (expansion + reranking)", { timeout: 90000 }, () => {    test("expands query with typed variations", async () => {
...
describe.skipIf(!!process.env.CI)("MCP HTTP Transport", () => {
  ...
  test("POST /mcp initialize returns 200 JSON (not SSE)", async () => {
    ...
    expect(json.result.serverInfo.name).toBe("mineru-document-explorer");
  });
