...
import { parseStructuredQuery as parseStructuredQueryFull } from "../src/query-parser.js";

// Wrapper that returns just the searches array for backward compat with existing test assertions
function parseStructuredQuery(query: string): ExpandedQuery[] | null {
  const result = parseStructuredQueryFull(query);
  return result ? result.searches : null;
}
...
describe("buildFTS5Query (lex parser)", () => {
  ...
  test("special chars in terms become word separators", () => {
    expect(buildFTS5Query("hello!world")).toBe('"hello world"*');
  });

  test("hyphenated terms produce phrase-prefix queries", () => {
    expect(buildFTS5Query("state-of-the-art")).toBe('"state of the art"*');
  });

  test("underscored terms produce phrase-prefix queries", () => {
    expect(buildFTS5Query("hello_world")).toBe('"hello world"*');
  });
});
