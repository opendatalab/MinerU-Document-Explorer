...
describe("Snippet Extraction", () => {
  ...
  test("extractSnippet respects maxLen for content", () => {
    ...
    expect(result.snippet.length).toBeLessThanOrEqual(103); // maxLen + "..."
  });
  ...
  test("extractSnippet returns clean snippet without diff header", () => {
    const body = "Line 1\nLine 2\nLine 3 has keyword\nLine 4\nLine 5";
  ...
  test("extractSnippet metadata is accurate for positioned snippet", () => {
    const body = "A\nB\nC keyword\nD\nE\nF\nG\nH";
    const { snippet, linesBefore, linesAfter, snippetLines } = extractSnippet(body, "keyword", 500);

    expect(snippet).not.toMatch(/^@@/);
    expect(snippet).toContain("C keyword");
    expect(linesBefore).toBe(1);    // A is before
    expect(snippetLines).toBe(4);   // 4 lines: B, C keyword, D, E
    expect(linesAfter).toBe(3);     // F, G, H are after
  });
...
describe("Fuzzy Matching", () => {
  test("findSimilarFiles finds similar paths", async () => {
    ...
    expect(similar.some(s => s.endsWith("docs/readme.md"))).toBe(true);
  ...
  test("findSimilarFiles respects maxDistance", async () => {
    ...
    expect(similar.some(s => s.endsWith("abc.md"))).toBe(true);
    expect(similar.every(s => !s.endsWith("xyz.md"))).toBe(true);

    await cleanupTestDb(store);
  });

  test("findSimilarFiles matches basename substring when full path distance is large", async () => {
    const store = await createTestStore();
    const collectionName = await createTestCollection();

    await insertTestDocument(store.db, collectionName, {
      name: "principles",
      displayPath: "mydocs/api-design-principles.md",
    });

    const similar = store.findSimilarFiles("api-design.md", 3, 5);
    expect(similar.some(s => s.endsWith("mydocs/api-design-principles.md"))).toBe(true);
...
describe.skipIf(!!process.env.CI)("LlamaCpp Integration", { timeout: 90000 }, () => {
  ...
  // Regression test for https://github.com/opendatalab/MinerU-Document-Explorer/pull/23
  ...
  test("rerank scores identical chunks per-file independently", async () => {
    const store = await createTestStore();
    ...
    try {
      ...
      // Both docs sent to reranker (not deduplicated by text)
      expect(rerankSpy).toHaveBeenCalledTimes(1);
      expect(rerankSpy.mock.calls[0]?.[1]).toHaveLength(2);
      // Each file gets its own score
      expect(first[0]!.file).toBe("doc1.md");
      expect(first[0]!.score).toBe(1);
      expect(first[1]!.file).toBe("doc2.md");
      expect(first[1]!.score).toBe(0.9);
      // Second call uses cache — no additional LLM calls
      expect(rerankSpy).toHaveBeenCalledTimes(1);
    } finally {
      llmSpy.mockRestore();
      await cleanupTestDb(store);
